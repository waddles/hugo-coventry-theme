var AT_Main = {
    ...
    ,stickMenu : function() {
        ...
        jQuery(window).scroll(function() {
            ...
            var smallScreen = jQuery(".header-mobile").is(':visible');
            container.find('.dropdown.hover').each(function () {
                if (jQuery(this).css('opacity') > 0) {
                    openSubMenus = true;
                }
            });
            if (scrollTop > lastScroll) { // downward-scrolling
                // only display sticky_menu when scrolling if there are no submenus open
                if (scrollTop > 250 && !openSubMenus && !smallScreen) {
                ...
                if (!openSubMenus) container.css({top: scrollTop});
                if (scrollTop < 250) {
                    container.css({
                        'z-index': -100,
                        opacity: 0,
    ...
    ,effectNavigation : function(){ // Make hover effect of navigation
        jQuery(".horizontal-menu .navigation .navbar-collapse .main-nav>li").hoverIntent(function(e){
            jQuery(this).find('>.dropdown-menu, >.dropdown-menu li >.dropdown-menu').addClass("animated");
        },function(e){
            jQuery(this).find('>.dropdown-menu, >.dropdown-menu li >.dropdown-menu').removeClass("animated");
        });

        jQuery(".top-account-holder").hoverIntent(function(e){
            jQuery(this).find('>.dropdown-menu').addClass("fadeInUp animated");
        },function(e){
            jQuery(this).find('>.dropdown-menu').removeClass("fadeInUp animated");
        });

        jQuery(".currency-block").hoverIntent(function(e){
            jQuery(this).find('>.dropdown-menu').addClass("fadeInUp animated");
        },function(e){
            jQuery(this).find('>.dropdown-menu').removeClass("fadeInUp animated");
        });
    ...
    ,menuOnMobile : function(){
        ...
        jQuery(".navbar .main-nav li").hoverIntent(function(){jQuery(this).addClass("hover")},function(){jQuery(this).removeClass("hover")});

        jQuery(".mobile-version .menu-mobile .main-nav .expand").addClass("fa fa-angle-right");
